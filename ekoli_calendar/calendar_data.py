...
class CalendarData:
    ...
    def load_calendar(self, filename: str) -> Dict:
        data = pd.read_csv(
            "https://docs.google.com/spreadsheets/d/1xCOMX6TxCMvDg4h1fXWnwdsSmMz5YJNiJvuLrpCd7oI/export?gid=0&format=csv",
            parse_dates=["Datum"]
        )
        # ekoli_data = defaultdict(lambda: defaultdict(dict))
        nested_dict = lambda: defaultdict(nested_dict)
        ekoli_data = nested_dict()

        ekoli_data["tasks"]["repetition"] = []
        ekoli_data["tasks"]["hidden_repetition"] = {}
        ekoli_data["name"] = "Ekoli Calendar"
        for row in data.iterrows():
            rd = row[1]
            year, month, day = rd["Datum"].year, rd["Datum"].month, rd["Datum"].day
            row_data = {
                "color": "#fc7967" if isinstance(rd["Voldoende Volk?"], float) or rd["Voldoende Volk?"].lower() == "nee" else "#29ccc4",
                "details": f"Locatie: {rd['Locatie']}<br />Uur: {rd['Uur']}<br />Leeftijd: {rd['Leeftijd']}",
                "title": "Geen Titel" if isinstance(rd["Project"], float) else rd["Project"],
                "start_time": "00:00",
                "end_time": "00:00",
                "id": str(uuid.uuid4()),
                "is_all_day": True
            }
            event = ekoli_data["tasks"]["normal"][str(year)][str(month)][str(day)]
            if event:
                event.append(row_data)
            else:
                ekoli_data["tasks"]["normal"][str(year)][str(month)][str(day)] = [row_data]
